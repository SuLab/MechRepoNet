...
def build_explanitory_graph(list_of_paths, list_of_edges=None, path_weights=None, edge_weights=None,
    ...
    node_info['label_color'] = node_info['color'].apply(lambda c: '#ffffff' if
                                                                  determine_text_color(c) == 'w' else '#000000')
...
def draw_explanitory_graph(G, node_id_to_name=None, proba=None, node_names=0, n_paths=10, xscale=10, max_line_len=15):
    """
    ...
    # Add probabilities
    if type(node_names) is int:

        first_node = list(G.nodes)[node_names]
